...
**SmartDeerFlow** 实现了先进的上下文工程系统，遵循 Manus AI 原则，实现最优的 LLM 上下文管理和效率。

### 概述

上下文工程系统通过多个专业组件提供统一的上下文管理：

| 组件 | 用途 | 核心原则 |
|------|------|----------|
| **FileBasedMemory** | 文件系统作为无限外部存储 | 突破 Token 限制持久化上下文 |
| **AttentionManager** | 通过重述进行注意力操控 | 控制模型关注区域 |
| **KVCacheOptimizer** | 稳定前缀的 KV-Cache 优化 | 最大化缓存命中率 |
| **SmartCompressor** | 智能上下文压缩 | 保留关键信息 |
| **DiversityInjector** | 避免少样本陷阱 | 防止模式固化 |
| **ErrorRecovery** | 保留错误内容用于学习 | 从失败中学习 |

### 核心特性

- 📁 **文件外部存储** - 使用文件系统作为无限上下文存储
- 🎯 **注意力操控** - 通过策略性重述控制模型焦点
- ⚡ **KV-Cache 优化** - 稳定前缀设计实现最大缓存效率
- 🗜️ **智能压缩** - 保留关键信息的智能内容压缩
- 🎲 **多样性注入** - 通过受控变化防止模式固化
- 🔄 **错误恢复** - 在上下文中保留失败尝试以实现隐式学习
- 🎭 **工具掩码** - 通过 logit 掩码约束动作，而非移除
...
**SmartDeerFlow** 实现了基于 GFLQ（生成-反馈-学习-查询）的先进反射机制，包含新的 **Reflexion Agent**：
...
> 📖 **详细文档：** [GFLQ 反射机制](./docs/GFLQ_REFLECTION_zh.md)
