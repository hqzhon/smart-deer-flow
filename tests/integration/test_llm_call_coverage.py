        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for pattern in llm_patterns:
                if re.search(pattern, line):
                    ...
                    if any(safe_word in line for safe_word in ['safe_llm_call', 'Mock', 'mock', 'test']):
                        continue
                        
                    # Check if it's a real LLM call by looking for common LLM variable names
                    if any(llm_var in line for llm_var in ['llm.', 'model.', 'chat.', 'openai.', 'deepseek.']):
                        findings.append({
                            'file': file_path,
                            'line': line_num,
                            'content': line.strip(),
    ...
    for file_pattern in critical_files:
        ...
        if os.path.exists(file_path):
            ...
            if file_pattern in instance_only_files:
                has_safe_import = True  # Consider these as "safe" since they don't make calls
                print(f"  Safe import: ✅ (instance-only file)")
            else:
...
def test_context_management_integration():
    ...
    try:
        ...
        from src.llms.error_handler import error_handler, safe_llm_call, safe_llm_call_async
        ...
        class MockLLM:
            def invoke(self, messages):
                ...
                if total_chars > 65536:
                    raise Exception("This model's maximum context length is 65536 tokens. However, you requested 300000 tokens")
...
def main():
    ...
    for file_path, result in results.items():
        ...
        if result.get('direct_calls', []):
            print(f"⚠️  {file_path} has {len(result['direct_calls'])} potential direct calls")
            issues_found = True
