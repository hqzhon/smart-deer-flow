...
class TestReflectionIntegration:
    ...
    def reflection_agent(self, config, reflection_config, mock_metrics):
        ...
        config.reflection_temperature = getattr(reflection_config, 'reflection_temperature', 0.7)
        config.reflection_confidence_threshold = getattr(reflection_config, 'knowledge_gap_threshold', 0.6)
    ...
    def planner_node(self, config, mock_metrics, reflection_agent):
        ...
        async def plan_with_reflection(query):
            return {
                'initial_plan': {'research_steps': [{'step': 1, 'action': 'Mock planning step'}]},
                'reflection_analysis': reflection_agent.analyze_knowledge_gaps.return_value,
                'enhanced_plan': {'research_steps': [{'step': 1, 'action': 'Mock planning step'}, {'step': 2, 'action': 'Enhanced step'}]}
            }
    ...
    @pytest.mark.asyncio
    async def test_researcher_reflection_integration(self, researcher_node, sample_research_query):
        """Test integration between researcher node and reflection agent."""
        with patch.object(researcher_node, '_perform_research', new_callable=AsyncMock) as mock_research:
            with patch.object(researcher_node.reflection_agent, 'analyze_knowledge_gaps', new_callable=AsyncMock) as mock_reflection:
                ...
                mock_reflection.return_value = ReflectionResult(
                    ...
                    follow_up_queries=['quantum computing recent advances', 'quantum supremacy 2024']
                )
                
                # Perform research with reflection
                result = await researcher_node.research_with_reflection(sample_research_query)
    ...
    @pytest.mark.asyncio
    async def test_planner_reflection_integration(self, planner_node, sample_research_query):
        """Test integration between planner node and reflection agent."""
        with patch.object(planner_node, '_generate_research_plan', new_callable=AsyncMock) as mock_planning:
            with patch.object(planner_node.reflection_agent, 'analyze_knowledge_gaps', new_callable=AsyncMock) as mock_reflection:
                # Mock initial plan
                ...
                mock_reflection.return_value = ReflectionResult(
                    ...
                    follow_up_queries=['quantum computing startups', 'quantum computing industry']
                )
                ...
                assert len(enhanced_plan['research_steps']) > 2  # Should have additional steps
    
    @pytest.mark.asyncio
    async def test_end_to_end_reflection_workflow(self, researcher_node, planner_node, sample_research_query):
        """Test complete end-to-end reflection workflow."""
        with patch.object(planner_node, '_generate_research_plan', new_callable=AsyncMock) as mock_planning:
            with patch.object(researcher_node, '_perform_research', new_callable=AsyncMock) as mock_research:
                with patch.object(planner_node.reflection_agent, 'analyze_knowledge_gaps', new_callable=AsyncMock) as mock_plan_reflection:
                    with patch.object(researcher_node.reflection_agent, 'analyze_knowledge_gaps', new_callable=AsyncMock) as mock_research_reflection:
                        
                        # Mock planning phase
                        mock_planning.return_value = {
                            'research_steps': [
                                {'step': 1, 'action': 'Search quantum computing basics'},
                        ...
                        plan_result = await planner_node.plan_with_reflection(sample_research_query)
                        research_result = await researcher_node.research_with_reflection(sample_research_query)
                        
                        # Verify both phases completed successfully
                        assert plan_result['reflection_analysis'].is_sufficient is True
                        assert research_result['reflection_analysis'].is_sufficient is True
    ...
    @pytest.mark.asyncio
    async def test_reflection_loop_termination(self, researcher_node, sample_research_query):
    ...
    @pytest.mark.asyncio
    async def test_reflection_error_handling(self, researcher_node, sample_research_query):
        ...
        with patch.object(researcher_node, '_perform_research', new_callable=AsyncMock) as mock_research:
            with patch.object(researcher_node.reflection_agent, 'analyze_knowledge_gaps', new_callable=AsyncMock) as mock_reflection:
                ...
                mock_reflection.side_effect = Exception("Reflection service unavailable")
    ...
    @pytest.mark.asyncio
    async def test_reflection_performance_metrics(self, researcher_node, sample_research_query, mock_metrics):
    ...
    @pytest.mark.asyncio
    async def test_reflection_caching_across_nodes(self, researcher_node, planner_node, sample_research_query):
        ...
        with patch('src.utils.reflection.enhanced_reflection.safe_llm_call_async', new_callable=AsyncMock) as mock_safe_llm:
            ...
            context = ReflectionContext(
                ...
                completed_steps=[{'step': 'initial_research', 'description': 'Test findings'}],
                execution_results=['Test findings'],
                current_step={'step': 'current', 'description': 'Current step'}
            )
            
            # First call from researcher node
            result1 = await researcher_node.reflection_agent.analyze_knowledge_gaps(context)
            
            # Second call from planner node with same context
            result2 = await planner_node.reflection_agent.analyze_knowledge_gaps(context)
    ...
    @pytest.mark.asyncio
    async def test_reflection_configuration_impact(self, config, mock_metrics, sample_research_query):
            ...
            return {
                ...
                'reflection_analysis': await strict_agent.analyze_knowledge_gaps(
                    ReflectionContext(
                        ...
                        completed_steps=[{'step': 'initial_research', 'description': 'Basic findings'}],
        ...
        with patch.object(strict_researcher, '_perform_research', new_callable=AsyncMock) as mock_strict_research:
            with patch.object(lenient_researcher, '_perform_research', new_callable=AsyncMock) as mock_lenient_research:
                with patch.object(strict_agent, '_call_reflection_model', new_callable=AsyncMock) as mock_strict_llm:
                    with patch.object(lenient_agent, '_call_reflection_model', new_callable=AsyncMock) as mock_lenient_llm:
                        ...
                        strict_result = await strict_researcher.research_with_reflection(sample_research_query)
                        lenient_result = await lenient_researcher.research_with_reflection(sample_research_query)
                        
                        # Verify different behaviors based on configuration
                        assert strict_result['reflection_analysis'].is_sufficient is False
                        assert lenient_result['reflection_analysis'].is_sufficient is True
