...
from src.utils.common.search_result_filter import (
    SearchResultCleaner,
    SearchResultFilter,
)
    ...
    def test_noise_pattern_removal(self):
        ...
        for case in test_cases:
            with self.subTest(case=case["name"]):
                ...
                # 检查清洗后的文本不为空
                self.assertGreater(
                    len(cleaned.strip()), 0, f"清洗后文本不应为空: {case['name']}"
                )
...
class TestSearchResultFilter(unittest.TestCase):
    ...
    def test_integrated_filtering(self):
        ...
        processed_results = self.filter._process_search_results_with_config(
            search_results
        )
        ...
        self.assertLess(
            len(processed_results), len(search_results), "应该过滤掉低质量结果"
        )
