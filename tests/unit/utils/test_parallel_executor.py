...
class TestCreateParallelExecutor:
    ...
    def test_create_parallel_executor_default(self, mock_load_config, mock_rate_limiter):
    ...
    @patch('src.utils.parallel_executor.get_global_rate_limiter')
    @patch('src.config.config_loader.config_loader.load_config')
    def test_create_parallel_executor_config_error(self, mock_load_config, mock_rate_limiter):
...
class TestParallelTaskIntegration:
    ...
    async def test_parallel_execution_with_deduplication(self):
        ...
        with patch('src.utils.parallel_executor.get_global_rate_limiter') as mock_rate_limiter:
            mock_limiter = Mock()
            mock_limiter.acquire = AsyncMock()
            mock_limiter.get_stats.return_value = {"total_delays": 0, "average_delay": 0.0}
            mock_rate_limiter.return_value = mock_limiter
    ...
    async def test_content_similarity_detection(self):
        ...
        # Add similar content
        content1 = "The research shows that artificial intelligence is transforming healthcare"
        ...
        assert not shared_context.is_content_duplicate(content3, similarity_threshold=0.3)