...
from .i18n import get_i18n_manager, Language, get_text
...
class LimitationDetector:
    ...
    def detect_limitations(self, content: str, metadata: Dict[str, Any]) -> List[DataLimitation]:
    ...
    def _detect_sample_limitations(self, content: str) -> List[DataLimitation]:
        ...
        patterns = self.detection_patterns.get(self.language, {}).get(LimitationType.SAMPLE_SIZE, [])
        for pattern in patterns:
            ...
            for match in matches:
                try:
                    ...
                    if sample_size < self.quality_indicators["small_sample"]["threshold"]:
                        limitations.append(DataLimitation(
                            limitation_id=str(uuid.uuid4())[:8],
                            limitation_type=LimitationType.SAMPLE_SIZE,
                            description=self.i18n.get_text("limitation_descriptions", "small_sample_size", sample_size=sample_size),
                            impact_level="high" if sample_size < 10 else "medium",
                            mitigation_suggestions=[
                                self.i18n.get_text("mitigation_suggestions", "increase_sample_size"),
                                self.i18n.get_text("mitigation_suggestions", "meta_analysis"),
                                self.i18n.get_text("mitigation_suggestions", "specify_scope")
                            ]
                        ))
                except ValueError:
                    continue
    ...
    def _detect_time_limitations(self, content: str) -> List[DataLimitation]:
        ...
        for pattern in short_period_patterns:
            if re.search(pattern, content, re.IGNORECASE):
                limitations.append(DataLimitation(
                    ...
                    description=self.i18n.get_text("limitation_descriptions", "short_time_span"),
                    impact_level="medium",
                    mitigation_suggestions=[
                        self.i18n.get_text("mitigation_suggestions", "extend_time_range"),
                        self.i18n.get_text("mitigation_suggestions", "explain_short_term_limits"),
                        self.i18n.get_text("mitigation_suggestions", "compare_historical_trends")
                    ]
                ))
                break
        ...
        if years:
            if self.language == Language.ZH_CN:
                ...
                latest_year = max(int(match[0]) if isinstance(match, tuple) else int(match) for match in years)
            ...
            if current_year - latest_year > 2:
                limitations.append(DataLimitation(
                    limitation_id=str(uuid.uuid4())[:8],
                    limitation_type=LimitationType.TIME_PERIOD,
                    description=self.i18n.get_text("limitation_descriptions", "outdated_data", year=latest_year),
                    impact_level="medium",
                    mitigation_suggestions=[
                        self.i18n.get_text("mitigation_suggestions", "update_data"),
                        self.i18n.get_text("mitigation_suggestions", "explain_timeliness_limits"),
                        self.i18n.get_text("mitigation_suggestions", "analyze_time_impact")
                    ]
                ))
    ...
    def _detect_geographic_limitations(self, content: str) -> List[DataLimitation]:
        ...
        if self.language == Language.ZH_CN:
            ...
            geographic_indicators = [
                "limited to China", "only in US", "confined to", "restricted to.*region",
                "single market", "specific region", "regional"
            ]
        ...
        for indicator in geographic_indicators:
            if re.search(indicator, content, re.IGNORECASE):
                limitations.append(DataLimitation(
                    ...
                    description=self.i18n.get_text("limitation_descriptions", "geographic_limitation"),
                    impact_level="medium",
                    mitigation_suggestions=[
                        self.i18n.get_text("mitigation_suggestions", "extend_geographic_scope"),
                        self.i18n.get_text("mitigation_suggestions", "cross_regional_study"),
                        self.i18n.get_text("mitigation_suggestions", "clarify_geographic_applicability")
                    ]
                ))
                break
    ...
    def _detect_methodology_limitations(self, content: str) -> List[DataLimitation]:
        ...
        if self.language == Language.ZH_CN:
            ...
            single_method_indicators = [
                "only use", "solely adopt", "single method", "based on.*model"
            ]
        ...
        for indicator in single_method_indicators:
            if re.search(indicator, content, re.IGNORECASE):
                limitations.append(DataLimitation(
                    ...
                    description=self.i18n.get_text("limitation_descriptions", "single_methodology"),
                    impact_level="medium",
                    mitigation_suggestions=[
                        self.i18n.get_text("mitigation_suggestions", "multiple_methods"),
                        self.i18n.get_text("mitigation_suggestions", "explain_methodology_limits"),
                        self.i18n.get_text("mitigation_suggestions", "sensitivity_analysis")
                    ]
                ))
                break
    ...
    def _detect_metadata_limitations(self, metadata: Dict[str, Any]) -> List[DataLimitation]:
        ...
        if len(data_sources) == 1:
            limitations.append(DataLimitation(
                ...
                description=self.i18n.get_text("limitation_descriptions", "single_data_source"),
                impact_level="medium",
                mitigation_suggestions=[
                    self.i18n.get_text("mitigation_suggestions", "multiple_data_sources"),
                    self.i18n.get_text("mitigation_suggestions", "assess_source_reliability"),
                    self.i18n.get_text("mitigation_suggestions", "explain_single_source_limits")
                ]
            ))
        ...
        if not metadata.get("has_control_group", True):
            limitations.append(DataLimitation(
                limitation_id=str(uuid.uuid4())[:8],
                limitation_type=LimitationType.METHODOLOGY,
                description=self.i18n.get_text("limitation_descriptions", "no_control_group"),
                impact_level="high",
                mitigation_suggestions=[
                    self.i18n.get_text("mitigation_suggestions", "setup_control_groups"),
                    self.i18n.get_text("mitigation_suggestions", "quasi_experimental_design"),
                    self.i18n.get_text("mitigation_suggestions", "clarify_correlation_causation")
                ]
            ))
...
class ConfidenceAssessor:
    ...
    def __init__(self, language: Language = Language.ZH_CN):
        ...
        self.confidence_indicators = {
            ...
            Language.EN_US: {
                ConfidenceLevel.VERY_HIGH: [
                    "multiple studies confirm", "large-scale randomized controlled trial", "meta-analysis shows",
                    "authoritative institution confirms", "consistent conclusions"
                ],
                ConfidenceLevel.HIGH: [
                    "multiple data sources", "repeated verification", "statistically significant",
                    "authoritative sources", "long-term tracking"
                ],
                ConfidenceLevel.MEDIUM: [
                    "preliminary research", "partial evidence", "limited sample",
                    "short-term observation", "single study"
                ],
                ConfidenceLevel.LOW: [
                    "theoretical speculation", "informal observation", "anecdotal evidence",
                    "expert opinion", "preliminary analysis"
                ],
                ConfidenceLevel.VERY_LOW: [
                    "speculation", "hypothesis", "possibly", "perhaps",
                    "no conclusion yet", "insufficient evidence"
                ]
            }
        }
    ...
    def assess_confidence(self, statement: str, supporting_evidence: List[str]) -> ConfidenceAnnotation:
        """Assess confidence level"""
        confidence_level = self._determine_confidence_level(statement, supporting_evidence)
        reasoning = self._generate_confidence_reasoning(statement, supporting_evidence, confidence_level)
    ...
    def _determine_confidence_level(self, statement: str, evidence: List[str]) -> ConfidenceLevel:
    ...
    def _generate_confidence_reasoning(self, statement: str, evidence: List[str], level: ConfidenceLevel) -> str:
        """Generate confidence reasoning"""
        evidence_count = len(evidence)
        
        reasoning_templates = {
            ConfidenceLevel.VERY_HIGH: self.i18n.get_text("confidence_reasoning", "very_high"),
            ConfidenceLevel.HIGH: self.i18n.get_text("confidence_reasoning", "high"),
            ConfidenceLevel.MEDIUM: self.i18n.get_text("confidence_reasoning", "medium"),
            ConfidenceLevel.LOW: self.i18n.get_text("confidence_reasoning", "low"),
            ConfidenceLevel.VERY_LOW: self.i18n.get_text("confidence_reasoning", "very_low")
        }
        
        template = reasoning_templates.get(level, self.i18n.get_text("confidence_reasoning", "error"))
...
class BiasDetector:
    ...
    def __init__(self, language: Language = Language.ZH_CN):
        ...
        self.bias_patterns = {
            ...
            Language.EN_US: {
                "confirmation_bias": [
                    "only consider.*support", "ignore.*contrary", "selective.*use",
                    "only focus.*positive", "exclude.*negative"
                ],
                "survivorship_bias": [
                    "success cases", "survivors", "only look.*surviving",
                    "ignore.*failure", "exclude.*delisted"
                ],
                "availability_bias": [
                    "recent cases", "easy to think", "media reports",
                    "impressive", "well-known cases"
                ],
                "anchoring_bias": [
                    "based on.*benchmark", "reference.*price", "based on.*expectation",
                    "anchored.*value", "influenced by.*"
                ]
            }
        }
    ...
    def detect_biases(self, content: str, methodology: Dict[str, Any]) -> List[BiasAssessment]:
        """Detect biases"""
        biases = []
    ...
    def _detect_methodology_biases(self, methodology: Dict[str, Any]) -> List[BiasAssessment]:
        """Detect methodology biases"""
        biases = []
        
        # Detect sample selection bias
        if methodology.get("sampling_method") == "convenience":
            biases.append(BiasAssessment(
                bias_type="selection_bias",
                severity="high",
                description=self.i18n.get_text("bias_types", "selection_bias"),
                potential_impact=self.i18n.get_text("potential_impacts", "selection_bias"),
                mitigation_measures=self.i18n.get_text("bias_mitigation", "selection_bias")
            ))
            
        # Detect temporal bias
        if methodology.get("data_period") and "crisis" in methodology.get("context", ""):
            biases.append(BiasAssessment(
                bias_type="temporal_bias",
                severity="medium",
                description=self.i18n.get_text("bias_types", "temporal_bias"),
                potential_impact=self.i18n.get_text("potential_impacts", "temporal_bias"),
                mitigation_measures=self.i18n.get_text("bias_mitigation", "temporal_bias")
            ))
...
class CriticalThinkingEngine:
    ...
    def analyze_content(self, content: str, metadata: Dict[str, Any] = None) -> CriticalAnalysis:
        ...
        # Detect biases
        biases = self.bias_detector.detect_biases(content, metadata.get("methodology", {}))
        ...
        quality_score = self._calculate_quality_score(limitations, biases, confidence_annotations)
    ...
    def _extract_key_statements(self, content: str) -> List[str]:
        ...
        if self.language == Language.ZH_CN:
            ...
            important_patterns = [
                r'结果显示', r'研究表明', r'数据证实', r'分析揭示',
                r'可以看出', r'表明', r'证明', r'显示'
            ]
        else:
            sentences = re.split(r'[.!?]', content)
            important_patterns = [
                r'results show', r'research indicates', r'data confirms', r'analysis reveals',
                r'can be seen', r'indicates', r'proves', r'demonstrates'
            ]
        ...
        for sentence in sentences:
            ...
            if any(re.search(pattern, sentence, re.IGNORECASE) for pattern in important_patterns):
    ...
    def _extract_supporting_evidence(self, statement: str, full_content: str) -> List[str]:
    ...
    def _enhance_content_with_critical_thinking(
        ...
        if limitations:
            enhanced += f"## {self.i18n.get_text('section_titles', 'data_limitations')}\n\n"
    ...
    def _calculate_quality_score(
        ...
        high_confidence_count = sum(1 for a in annotations 
