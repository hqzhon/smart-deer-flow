...
class TemplateValidator:
    ...
    def validate_content_block(self, block: ContentBlock, rules: List[str]) -> Dict[str, Any]:
        ...
        for rule in rules:
            if rule in self.validation_rules:
                try:
                    ...
                    validation_result["errors"].append(f"Validation rule exception: {rule}")
    ...
    def _validate_table_structure(self, block: ContentBlock) -> Dict[str, Any]:
        ...
        if isinstance(block.content, str):
            ...
            if len(lines) < 3:
                errors.append("Table requires at least header row, separator row, and data row")
            else:
                # Check table format
                ...
                if header_cols != separator_cols:
                    errors.append("Table header and separator row column count mismatch")
                    
                # Check data rows
                for i, line in enumerate(lines[2:], start=2):
                    data_cols = len(line.split('|'))
                    if data_cols != header_cols:
                        errors.append(f"Row {i+1} column count does not match header row")
            ...
            if "headers" not in block.content or "rows" not in block.content:
                ...
                for i, row in enumerate(block.content["rows"]):
                    if len(row) != header_count:
                        errors.append(f"Row {i+1} data column count does not match header")
    ...
    def _validate_financial_metrics(self, block: ContentBlock) -> Dict[str, Any]:
        ...
        financial_terms = [
            "revenue", "profit", "ebitda", "margin", "ratio", "growth",
            "income", "profit", "gross margin", "net margin", "year-over-year", "month-over-month"
        ]
    ...
    def _validate_academic_format(self, block: ContentBlock) -> Dict[str, Any]:
        ...
        if block.metadata.get("domain") == "academic":
            ...
            if "assumption" not in content.lower() and "limitation" not in content.lower():
                suggestions.append("Recommend stating research assumptions or limitations")
...
class ReportBuilder:
    ...
    def validate_report_content(self, template_id: str, content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for section in template.sections:
            ...
            if section_content:
                ...
                section_result = self._validate_section_content(section, section_content)
    ...
    def _validate_section_content(self, section: ReportSection, content: Any) -> Dict[str, Any]:
        """Validate section content"""
        validation_result = {
            "is_valid": True,
            "errors": [],
        ...
        if style_req.get("requires_charts", False):
            ...
            if not any(keyword in str(content).lower() for keyword in chart_keywords):
                validation_result["suggestions"].append("Recommend adding charts to enhance visualization")
                
        if style_req.get("format") == "bullet_points":
            if not ("•" in str(content) or "-" in str(content) or "*" in str(content)):
                validation_result["suggestions"].append("Recommend using bullet point format")
    ...
    def generate_report_template(self, template_id: str) -> str:
        ...
        # Add generation info
        markdown += f"**Generated Time**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        ...
        for section in template.sections:
            ...
            if section.word_limit:
                markdown += f"*Recommended word count: within {section.word_limit} words*\n\n"
    ...
    def apply_domain_specific_enhancements(self, content: str, domain: ReportDomain) -> str:
