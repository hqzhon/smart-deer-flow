...
class ContentItem(BaseModel):
    ...
    text: Optional[str] = Field(
        None, description="The text content if type is 'text'", max_length=50000
    )
...
class ChatRequest(BaseModel):
    ...
    enable_collaboration: Optional[bool] = Field(
        True,
        description="Whether to enable collaboration features (role bidding, human loop, conflict resolution)",
    )
...
class TTSRequest(BaseModel):
    text: str = Field(
        ..., description="The text to convert to speech", max_length=10000
    )
    voice_type: Optional[str] = Field(
        "BV700_V2_streaming", description="The voice type to use", max_length=50
    )
    encoding: Optional[str] = Field(
        "mp3", description="The audio encoding format", pattern=r"^(mp3|wav|ogg)$"
    )
    speed_ratio: Optional[float] = Field(
        1.0, description="Speech speed ratio", ge=0.1, le=3.0
    )
    volume_ratio: Optional[float] = Field(
        1.0, description="Speech volume ratio", ge=0.1, le=2.0
    )
    pitch_ratio: Optional[float] = Field(
        1.0, description="Speech pitch ratio", ge=0.1, le=2.0
    )
    text_type: Optional[str] = Field(
        "plain", description="Text type (plain or ssml)", pattern=r"^(plain|ssml)$"
    )
    with_frontend: Optional[int] = Field(
        1, description="Whether to use frontend processing", ge=0, le=1
    )
    frontend_type: Optional[str] = Field(
...
class GenerateProseRequest(BaseModel):
    ...
    option: str = Field(
        ..., description="The option of the prose writer", max_length=100
    )
...
class EnhancePromptRequest(BaseModel):
    prompt: str = Field(
        ..., description="The original prompt to enhance", max_length=50000
    )
