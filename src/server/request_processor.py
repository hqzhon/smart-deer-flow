...
connection_context: ContextVar[Optional[str]] = ContextVar(
...
class RequestContext:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            logger.debug(
                f"Acquired connection {self.connection_id} for request {self.request_data.id}"
            )
        except Exception as e:
            logger.error(
                f"Failed to acquire connection for request {self.request_data.id}: {e}"
            )
            raise
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.connection_id:
            try:
                ...
                logger.debug(
                    f"Released connection {self.connection_id} for request {self.request_data.id}"
                )
            except Exception as e:
...
class RAGQueryProcessor(RequestProcessor):
    ...
    async def _process_rag_query(
        self, request: RAGResourceRequest, context: RequestContext
    ) -> Dict[str, Any]:
...
class UnknownRequestProcessor(RequestProcessor):
    ...
    async def process(
        ...
        logger.warning(
            "Processing unknown request type",
            extra={
                "request_id": context.request_data.id,
...
class RequestProcessorFactory:
    ...
    def get_processor(cls, request_type: RequestType) -> RequestProcessor:
        ...
        if not processor:
            logger.warning(
                f"No specific processor found for {request_type}, using unknown processor"
            )
    ...
    def register_processor(cls, request_type: RequestType, processor: RequestProcessor):
        ...
        logger.info(
            f"Registered processor {processor.get_processor_name()} for type {request_type}"
        )
...
async def record_failure_metrics(
    request: BatchRequestData, context: RequestContext, error: str
):
