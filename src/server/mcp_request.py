...
class MCPServerMetadataRequest(BaseModel):
    ...
    transport: str = Field(
        ..., description="The type of MCP server connection (stdio or sse)", pattern=r"^(stdio|sse)$"
    )
    ...
    timeout_seconds: Optional[int] = Field(
        None, description="Optional custom timeout in seconds for the operation", ge=1, le=300
    )
