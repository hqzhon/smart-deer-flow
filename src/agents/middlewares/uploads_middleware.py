...
class UploadsMiddleware(AgentMiddleware[UploadsMiddlewareState]):
    ...
    def _list_newly_uploaded_files(
        self, thread_id: str, last_message_files: set[str]
    ) -> list[dict]:
    ...
    @override
    def before_agent(
        self, state: UploadsMiddlewareState, runtime: Runtime
    ) -> dict | None:
        ...
        logger.info(
            f"Original message content: {original_content[:100] if original_content else '(empty)'}"
        )
