...
class RoleBiddingSystem:
    ...
    def calculate_compatibility_score(
        self, agent: AgentCapability, task: TaskRequirement
    ) -> float:
        ...
        final_score = (
            base_score + success_weight + tool_match + domain_score - workload_penalty
        )
        return max(0.0, min(1.0, final_score))

    def generate_agent_bid(
        self, agent: AgentCapability, task: TaskRequirement
    ) -> Optional[Bid]:
        """Generate agent bid"""
    ...
    def _generate_bid_reasoning(
        self, agent: AgentCapability, task: TaskRequirement, compatibility: float
    ) -> str:
        """Generate bid reasoning"""
        reasons = []
    ...
    def conduct_bidding(self, task: TaskRequirement) -> List[Bid]:
        ...
        for agent in self.agents.values():
            ...
            if bid:
                bids.append(bid)
                logger.debug(
                    f"Agent {agent.agent_id} bid, confidence: {bid.confidence_score:.2f}"
                )
    ...
    def select_winner(self, task: TaskRequirement, bids: List[Bid]) -> Optional[str]:
        ...
        logger.info(
            f"Task {task.task_id} assigned to agent {winner_id}, score: {best_score:.2f}"
        )
    ...
    def complete_task(self, task_id: str, success: bool, actual_duration: int):
        ...
        if agent_id in self.agents:
            self.agents[agent_id].current_workload = max(
                0, self.agents[agent_id].current_workload - 1
            )
        ...
        logger.info(
            f"Task {task_id} completed, success: {success}, duration: {actual_duration} minutes"
        )
    ...
    def get_agent_statistics(self) -> Dict[str, Dict]:
        ...
        for agent_id, agent in self.agents.items():
            completed_tasks = [
                t for t in self.task_history if t["agent_id"] == agent_id
            ]
