...
class WorkflowMetrics:
    ...
    def record_execution(self, result: 'WorkflowResult') -> None:
        ...
        total_time = (self.metrics_data['average_execution_time'] * 
                     (self.metrics_data['total_executions'] - 1) + result.execution_time)
        self.metrics_data['average_execution_time'] = total_time / self.metrics_data['total_executions']
        ...
        for stage_name, stage_result in result.stage_results.items():
            ...
            if 'stage_metrics' in stage_result and 'execution_time' in stage_result['stage_metrics']:
                stage_time = stage_result['stage_metrics']['execution_time']
                total_stage_time = (stage_metrics['average_time'] * 
                                  (stage_metrics['executions'] - 1) + stage_time)
                stage_metrics['average_time'] = total_stage_time / stage_metrics['executions']
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        if self.metrics_data['total_executions'] > 0:
            success_rate = (self.metrics_data['successful_executions'] / 
                          self.metrics_data['total_executions']) * 100
...
class ReflectionWorkflow:
    ...
    async def execute_workflow(self, 
                             research_query: str,
                             initial_context: Optional[Dict[str, Any]] = None) -> WorkflowResult:
        ...
        try:
            ...
            final_metrics = await self._generate_final_metrics(stage_results, execution_time)
    ...
    async def _execute_stage(self, stage: WorkflowStage, stage_index: int) -> Dict[str, Any]:
    ...
    async def _execute_context_analysis_stage(self) -> Dict[str, Any]:
        ...
        knowledge_gaps = await self.reflection_agent.analyze_knowledge_gaps(reflection_context)
    ...
    async def _execute_research_planning_stage(self) -> Dict[str, Any]:
        ...
        context_analysis = self.execution_history[-1]["result"] if self.execution_history else {}
    ...
    async def _execute_information_gathering_stage(self) -> Dict[str, Any]:
        ...
        # Get research plan
        planning_result = self.execution_history[-1]["result"] if self.execution_history else {}
    ...
    async def _execute_synthesis_stage(self) -> Dict[str, Any]:
        ...
        gathering_result = self.execution_history[-1]["result"] if self.execution_history else {}
    ...
    async def _execute_validation_stage(self) -> Dict[str, Any]:
        ...
        synthesis_result = self.execution_history[-1]["result"] if self.execution_history else {}
    ...
    async def _apply_reflection(self, stage: WorkflowStage, stage_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Perform reflection
            reflection_insights = await self.reflection_agent.analyze_knowledge_gaps(reflection_context)
            
            return {
                "reflection_applied": True,
                "reflection_insights": [reflection_insights],
                "reflection_confidence": reflection_insights.get("confidence_score", 0.5)
            }
    ...
    async def _generate_final_metrics(self, stage_results: Dict[str, Any], execution_time: float) -> Dict[str, Any]:
        ...
        try:
            ...
            for stage_name, result in stage_results.items():
                ...
                if result.get("reflection_applied"):
                    total_reflection_time += stage_metrics[stage_name].get("execution_time", 0)
                if result.get("context_expanded"):
                    total_expansion_time += stage_metrics[stage_name].get("execution_time", 0)
            ...
            return {
                "workflow_metrics": {
                    ...
                    "stages_successful": sum(1 for r in stage_results.values() if r.get("success", False)),
                ...
                "performance_summary": {
                    "efficiency_score": min(1.0, 60.0 / execution_time) if execution_time > 0 else 1.0,
                    "success_rate": len([r for r in stage_results.values() if r.get("success", False)]) / len(stage_results) if stage_results else 0,
                    "reflection_coverage": sum(1 for r in stage_results.values() if r.get("reflection_applied", False)) / len(stage_results) if stage_results else 0
                }
            }
    ...
    def get_workflow_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "execution_history": self.execution_history[-3:] if self.execution_history else []  # Last 3 stages
        }
