...
def planner_node(
    ...
    reflection_config = {
        'enable_enhanced_reflection': getattr(configurable, 'enable_enhanced_reflection', True),
        ...
        'knowledge_gap_threshold': getattr(configurable, 'knowledge_gap_threshold', 0.7),
        'sufficiency_threshold': getattr(configurable, 'sufficiency_threshold', 0.8),
        'enable_reflection_integration': getattr(configurable, 'enable_reflection_integration', True)
    }
    ...
    if reflection_config.get('enable_enhanced_reflection', True):
        try:
            from src.utils.reflection.enhanced_reflection import EnhancedReflectionAgent, ReflectionConfig
    ...
    reflection_context = ""
    if reflection_insights and reflection_config.get('enable_reflection_integration', True):
...
REFLECTION INSIGHTS FROM PREVIOUS RESEARCH:
        ...
        if reflection_agent is not None:
            try:
                ...
                logger.info("Skipping planner-specific reflection (async not supported in sync function)")
...
{chr(10).join([f"- {gap}" for gap in planner_reflection.follow_up_queries]) if planner_reflection.follow_up_queries else "- No planning gaps identified"}
                    ...
                    logger.info(f"Planner reflection completed - Sufficient: {planner_reflection.is_sufficient}, Gaps: {len(planner_reflection.knowledge_gaps)}")
                elif planner_reflection is None:
                    logger.info("Planner reflection was skipped due to sync function limitations")
    ...
    if curr_plan.get("has_enough_context"):
        ...
        reflection_metadata = {
            'reflection_applied': reflection_config.get('enable_reflection_integration', True) and reflection_insights is not None,
        ...
        if reflection_insights:
            reflection_metadata.update({
                'knowledge_gaps_count': len(reflection_insights.get('knowledge_gaps', [])),
...
async def _execute_single_step_parallel(
    ...
    for s in current_plan.steps:
        if s.title == step.title:
            ...
            temp_steps.append(s.copy(update={"execution_res": "Pending parallel execution"}))
    ...
    try:
            ...
            if updated_plan:
                ...
                for updated_step in updated_plan.steps:
                    if updated_step.title == step.title and updated_step.execution_res:
                        ...
                        final_step = step.copy(update={"execution_res": response_content})
                        
                        logger.info(f"Parallel step '{step.title}' completed by {agent_type} through node")
        ...
        logger.warning(f"Could not extract proper result for step '{step.title}', using fallback")
        ...
        logger.error(f"Error executing step '{step.title}' through {agent_type} node: {e}")
        logger.error(f"Full traceback: {traceback.format_exc()}")
        ...
        return type(
            "StepResult", (), 
            {"content": error_content, "updated_step": error_step}
        )()
...
async def researcher_node_with_isolation(
    ...
    if reflection_config.get('enable_enhanced_reflection', False):
        logger.info(f"Enhanced reflection enabled for researcher_node: {reflection_config}")
        try:
            from src.utils.reflection.enhanced_reflection import EnhancedReflectionAgent, ReflectionConfig
    ...
    if mcp_servers:
        async with MultiServerMCPClient(mcp_servers) as client:
            ...
            isolation_result = await context_extension.execute_researcher_step_with_isolation(
    ...
    if reflection_agent is not None:
        try:
            ...
            enable_iterative_research = getattr(configurable, "enable_iterative_research", True)
            max_queries_per_iteration = getattr(configurable, "max_queries_per_iteration", 3)
            ...
            if not enable_iterative_research:
                logger.info("Iterative research is disabled, performing single reflection analysis")
                max_iterations = 1
            ...
            logger.info(f"Starting iterative research loop (max_iterations: {max_iterations}, threshold: {sufficiency_threshold})")
            
            while current_iteration < max_iterations:
                current_iteration += 1
                logger.info(f"Iteration {current_iteration}/{max_iterations}: Performing reflection analysis...")
                ...
                reflection_context = ReflectionContext(
                    ...
                    total_steps=len(current_plan.steps) if current_plan and hasattr(current_plan, 'steps') else 1,
                    current_step_index=len(completed_steps),
                    locale=state.get('locale', 'en-US')
                )
                
                # Perform reflection analysis
                reflection_result = await reflection_agent.analyze_knowledge_gaps(reflection_context)
                
                logger.info(f"Iteration {current_iteration}: Sufficient={reflection_result.is_sufficient}, Confidence={reflection_result.confidence_score}, Gaps={len(reflection_result.knowledge_gaps)}")
                
                # Check termination conditions
                if (reflection_result.is_sufficient or 
                    reflection_result.confidence_score >= sufficiency_threshold or 
                    not reflection_result.follow_up_queries or
                    current_iteration >= max_iterations):
                    
                    logger.info(f"Terminating iterative research: sufficient={reflection_result.is_sufficient}, confidence={reflection_result.confidence_score}, queries={len(reflection_result.follow_up_queries) if reflection_result.follow_up_queries else 0}")
                    break
                
                # Execute follow-up queries
                queries_to_execute = reflection_result.follow_up_queries[:max_queries_per_iteration]
                logger.info(f"Executing {len(queries_to_execute)} follow-up queries (limited from {len(reflection_result.follow_up_queries)} total)...")
                
                for i, query in enumerate(queries_to_execute):
                    try:
                        logger.info(f"Executing follow-up query {i+1}: {query[:100]}...")
                        ...
                        if hasattr(follow_up_result, 'update') and follow_up_result.update:
                            follow_up_observations = follow_up_result.update.get('observations', [])
                            follow_up_messages = follow_up_result.update.get('messages', [])
                            
                            # Add follow-up findings to accumulated research
                            for obs in follow_up_observations:
                                if isinstance(obs, dict) and 'content' in obs:
                                    all_research_findings.append(f"[Follow-up {current_iteration}.{i+1}] {obs['content']}")
                                    all_observations.append(obs)
                                elif isinstance(obs, str):
                                    all_research_findings.append(f"[Follow-up {current_iteration}.{i+1}] {obs}")
                                    all_observations.append({'content': obs, 'type': 'follow_up_research'})
                                else:
                                    content = str(obs)
                                    all_research_findings.append(f"[Follow-up {current_iteration}.{i+1}] {content}")
                                    all_observations.append({'content': content, 'type': 'follow_up_research'})
                            
                            for msg in follow_up_messages:
                                if isinstance(msg, dict) and 'content' in msg:
                                    all_research_findings.append(f"[Follow-up {current_iteration}.{i+1}] {msg['content']}")
                                elif hasattr(msg, 'content'):
                                    all_research_findings.append(f"[Follow-up {current_iteration}.{i+1}] {msg.content}")
                                else:
                                    all_research_findings.append(f"[Follow-up {current_iteration}.{i+1}] {str(msg)}")
            ...
            logger.info("Performing final reflection analysis with all accumulated findings...")
            ...
            final_reflection_result = await reflection_agent.analyze_knowledge_gaps(final_reflection_context)
            ...
            if hasattr(isolation_result, 'update') and isolation_result.update:
                ...
                isolation_result.update['reflection_insights'] = {
                    ...
                    'knowledge_gaps': [gap.to_dict() if hasattr(gap, 'to_dict') else gap for gap in final_reflection_result.knowledge_gaps],
                ...
                if not final_reflection_result.is_sufficient and final_reflection_result.follow_up_queries:
                    isolation_result.update['suggested_follow_up'] = final_reflection_result.follow_up_queries
                    logger.info(f"Final reflection identified {len(final_reflection_result.follow_up_queries)} additional follow-up queries")
            ...
            logger.info(f"Iterative research completed - Final Sufficient: {final_reflection_result.is_sufficient}, Iterations: {current_iteration}, Total Findings: {len(all_research_findings)}, Final Confidence: {final_reflection_result.confidence_score}")
            ...
            if hasattr(isolation_result, 'update') and isolation_result.update:
                isolation_result.update['reflection_metadata'] = {
                    ...
                    'reflection_timestamp': time.time(),
                    'iterative_research_enabled': True,
                    'error_during_iteration': current_iteration if 'current_iteration' in locals() else 0
                }
    ...
    if hasattr(isolation_result, 'update') and isolation_result.update:
        ...
        reflection_helpers = {
            ...
            'is_research_sufficient': isolation_result.update.get('reflection_insights', {}).get('is_sufficient', True),
            'reflection_insights': isolation_result.update.get('reflection_insights', {})
        }
