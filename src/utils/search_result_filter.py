...
"""
    ...
    def should_enable_smart_filtering(
        ...
        content_within_limit, token_result = (
            self.content_processor.check_content_token_limit(
                formatted_content, model_name, limits.input_limit, limits.safety_margin
            )
        )

        # Calculate smart filtering threshold
        smart_filtering_threshold = int(
            limits.input_limit * self.SMART_FILTERING_THRESHOLD_RATIO
        )
    ...
    def filter_search_results(
        ...
        prompt_within_limit, prompt_token_result = (
            self.content_processor.check_content_token_limit(
...
---"""
    ...
    def _single_filter_call(
        self, filter_prompt: str, llm: BaseLanguageModel, original_count: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if filtered_data and "filtered_results" in filtered_data:
                logger.info(
                    f"Successfully filtered: {len(filtered_data['filtered_results'])}/{original_count} results"
                )
    ...
    def _parse_json_response(self, response_content: str) -> Optional[Dict[str, Any]]:
        ...
        json_match = re.search(
            r'{[^{}]*"filtered_results"[^{}]*}', response_content, re.DOTALL
        )
        if json_match:
            try:
    ...
    def _batch_filter_results(
        ...
        batches = [
            search_results[i : i + self.DEFAULT_BATCH_SIZE]
        ...
        for i, batch in enumerate(batches):
            ...
            batch_prompt = self.filter_prompt_template.format(
                query=query, search_results=formatted_batch
            )
        ...
        if len(all_filtered_results) > self.MAX_SECONDARY_RESULTS:
            ...
            all_filtered_results = self._apply_secondary_filter(
                query, all_filtered_results, llm
            )

        combined_summary = (
            "\n\n".join(batch_summaries)
            if batch_summaries
            else "Batch filtering completed"
        )
    ...
    def _apply_secondary_filter(
        self, query: str, filtered_results: List[Dict[str, Any]], llm: BaseLanguageModel
    ) -> List[Dict[str, Any]]:
...
"""
            ...
            selected_indices = [
                int(n) - 1 for n in numbers if 1 <= int(n) <= len(filtered_results)
            ]

            if selected_indices:
                return [
                    filtered_results[i]
                    for i in selected_indices[: self.MAX_SECONDARY_RESULTS]
                ]
