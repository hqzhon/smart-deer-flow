...
class HierarchicalMemoryManager(Generic[T]):
    ...
    async def get(self, key: Union[str, Dict[str, Any]], default: T = None) -> Optional[T]:
    ...
    async def _set_in_l1(self, entry: CacheEntry) -> bool:
        ...
        with self.l1_lock:
            ...
            while (self.l1_current_size + entry.size_bytes > self.l1_max_size and 
                   len(self.l1_cache) > 0):
    ...
    async def _set_in_l2(self, entry: CacheEntry) -> bool:
        ...
        with self.l2_lock:
            # Check if we need to evict
            while (self.l2_current_size + entry.size_bytes > self.l2_max_size and 
                   len(self.l2_cache) > 0):
    ...
    async def _set_in_l3(self, entry: CacheEntry) -> bool:
        ...
        with self.l3_lock:
            # Check if we need to evict
            while (self.l3_current_size + entry.size_bytes > self.l3_max_size and 
                   len(self.l3_cache) > 0):
    ...
    async def _cleanup_expired_entries(self):
        ...
        # Clean L2
        with self.l2_lock:
            expired_keys = [key for key, entry in self.l2_cache.items() 
                           if entry.is_expired()]
            for key in expired_keys:
                self._remove_from_l2(key)
        ...
        # Clean L3
        with self.l3_lock:
            expired_keys = [key for key, entry in self.l3_cache.items() 
                           if entry.is_expired()]
            for key in expired_keys:
                self._remove_from_l3(key)
            ...
            def sync_wrapper(*args, **kwargs):
                ...
                with self._sync_cache_lock:
                    if cache_key in self._sync_cache:
                        ...
                        # Check if expired
                        if self.ttl is None or (current_time - cached_item['timestamp']) < self.ttl:
                ...
                with self._sync_cache_lock:
                    ...
                    if len(self._sync_cache) > 100:
                        # Delete oldest item
                        oldest_key = min(self._sync_cache.keys(), 
