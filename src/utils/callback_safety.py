...
class SafeCallbackManager:
    ...
    def register_callback(self, event_type: str, callback: Optional[Callable], callback_name: str = None):
        """Register callback function"""
        if callback is None:
            logger.warning(f"Attempted to register None callback for event {event_type}")
            return
        
        if not callable(callback):
            logger.error(f"Attempted to register non-callable object for event {event_type}")
            return
        ...
        logger.debug(f"Registered callback {callback_name or 'anonymous'} for event {event_type}")
    ...
    async def safe_call_async(self, event_type: str, *args, **kwargs) -> List[CallbackResult]:
        ...
        # Execute all callbacks
        for i, callback in enumerate(callbacks):
            result = await self._execute_callback_async(callback, event_type, i, *args, **kwargs)
            results.append(result)
    ...
    def _execute_callback(self, callback: Callable, event_type: str, index: int, *args, **kwargs) -> CallbackResult:
    ...
    async def _execute_callback_async(self, callback: Callable, event_type: str, index: int, *args, **kwargs) -> CallbackResult:
...
def safe_callback(event_type: str, callback_name: str = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                if func is None:
                    logger.warning(f"Attempted to call None function for event {event_type}")
                    return None
        ...
        async def async_wrapper(*args, **kwargs):
            try:
                ...
                logger.error(f"Async callback {callback_name or func.__name__} failed: {e}")
                return None
...
def ensure_callback_safety(callback: Optional[Callable], default_callback: Optional[Callable] = None) -> Optional[Callable]:
