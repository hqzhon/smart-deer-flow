...
class ConfigWatcher(FileSystemEventHandler):
    ...
    def __init__(self, config_manager: 'ConfigManager', watched_files: Dict[str, ConfigSource]):
    ...
    def on_modified(self, event):
        ...
        if file_path in self._last_modified:
            if current_time - self._last_modified[file_path] < 1.0:  # Ignore duplicate events within 1 second
                return
        
        self._last_modified[file_path] = current_time
        
        logger.info(f"Configuration file changed: {file_path}", event_type=EventType.SYSTEM)
        
        try:
            self.config_manager.reload_config()
        except Exception as e:
            logger.error(f"Failed to reload configuration: {e}", error=e, event_type=EventType.ERROR)
...
class ConfigManager:
        ...
        with self._lock:
            ...
            for source in reversed(self.sources):  # Load low priority first
                try:
                    ...
                    if source.required:
                        logger.error(f"Failed to load required configuration from {source.path}: {e}",
    ...
    def _load_source(self, source: ConfigSource) -> Optional[Dict[str, Any]]:
        ...
        if source.format != ConfigFormat.ENV:
            if not os.path.exists(source.path):
                if source.required:
                    raise FileNotFoundError(f"Configuration file not found: {source.path}")
                else:
                    return None
    ...
    def _validate_config(self, config: Dict[str, Any]):
        ...
        for validator in self.validators:
            try:
                ...
                logger.error(f"Configuration validation error: {e}", error=e, event_type=EventType.ERROR)
                raise
    ...
    def _trigger_change_callbacks(self, config: Dict[str, Any]):
        ...
        for callback in self.change_callbacks:
            try:
                ...
                logger.error(f"Configuration change callback error: {e}", error=e, event_type=EventType.ERROR)
    ...
    def _setup_file_watching(self):
        ...
        for source in self.sources:
            if source.watch and source.format != ConfigFormat.ENV and os.path.exists(source.path):
        ...
        logger.info(f"Started watching {len(watch_files)} configuration files", event_type=EventType.SYSTEM)
    ...
    def set(self, key: str, value: Any):
        ...
        with self._lock:
            ...
            logger.debug(f"Configuration value updated: {key}", 
                        event_type=EventType.SYSTEM,
    ...
    def stop_watching(self):
        ...
        if self._observer:
            ...
            self._observer.join()
            self._observer = None
            logger.info("Stopped configuration file watching", event_type=EventType.SYSTEM)
...
def config_property(key: str, default: Any = None, config_manager: ConfigManager = None):
...
def get_config_manager() -> ConfigManager:
    ...
    if global_config_manager is None:
        ...
        try:
            ...
            logger.warning(f"Unknown environment '{env_name}', using development", event_type=EventType.SYSTEM)
...
def configure_from_files(*file_paths: str, watch: bool = True):
    ...
    for file_path in file_paths:
        path = Path(file_path)
        if not path.exists():
            logger.warning(f"Configuration file not found: {file_path}", event_type=EventType.SYSTEM)
            continue
        ...
        if not file_format:
            logger.warning(f"Unknown configuration file format: {file_path}", event_type=EventType.SYSTEM)
            continue
