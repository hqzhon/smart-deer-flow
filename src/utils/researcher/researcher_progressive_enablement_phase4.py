...
from .researcher_progressive_enablement import (
    TaskComplexity, IsolationStrategy, ScenarioContext, 
...
class AdvancedScenarioFeatures:
    ...
    # Temporal features
    time_of_day: int = field(default_factory=lambda: int(time.time() % 86400))  # Seconds since midnight
...
class DynamicThreshold:
    ...
    def adjust(self, performance_feedback: float):
        ...
        older_avg = statistics.mean(list(self.performance_history)[-20:-10:]) if len(self.performance_history) >= 20 else recent_avg
        ...
        logger.debug(f"Adjusted threshold {self.name}: {self.current_value:.2f} (trend: {trend:.3f})")
...
class AdvancedResearcherProgressiveEnabler(BaseEnabler):
    ...
    def __init__(self):
        ...
        self.dynamic_thresholds = {
            ...
            "token_estimate": DynamicThreshold("token_estimate", 10000.0, 3000.0, 30000.0),
        ...
        self.pattern_cache: Dict[str, Tuple[bool, float, float]] = {}  # pattern -> (decision, confidence, timestamp)
        ...
        logger.info("Initialized AdvancedResearcherProgressiveEnabler with ML-style optimization")
    
    def extract_advanced_features(self, scenario: ScenarioContext) -> AdvancedScenarioFeatures:
        ...
        complexity_keywords = {
            "analyze": 2, "compare": 2, "synthesize": 3, "evaluate": 2,
            "comprehensive": 2, "detailed": 1, "thorough": 2, "in-depth": 3,
            "multiple": 1, "various": 1, "different": 1, "cross-reference": 3
        }
        
        features.query_complexity_score = sum(
        ...
        features.resource_pressure = min(1.0, recent_sessions / 10.0)  # Normalize to 0-1
    ...
    def calculate_isolation_score(self, features: AdvancedScenarioFeatures) -> float:
        ...
        if scenario.context_size >= self.dynamic_thresholds["context_size"].current_value:
            score += self.feature_weights["context_size"]
        
        if scenario.estimated_tokens >= self.dynamic_thresholds["token_estimate"].current_value:
            score += self.feature_weights["token_estimate"]
        
        if features.query_complexity_score >= self.dynamic_thresholds["complexity_score"].current_value:
    ...
    def predict_performance_impact(self, features: AdvancedScenarioFeatures, enable_isolation: bool) -> Tuple[float, float]:
        """Predict performance impact of isolation decision"""
        return self.performance_predictor.predict(features, enable_isolation)
    
    def should_enable_isolation_advanced(self, scenario: ScenarioContext) -> Tuple[bool, str, Dict[str, Any], DecisionConfidence]:
        ...
        if pattern_key in self.pattern_cache:
            cached_decision, cached_confidence, timestamp = self.pattern_cache[pattern_key]
            if time.time() - timestamp < 3600:  # Cache valid for 1 hour
                return cached_decision, "Cached decision from similar pattern", {"pattern_key": pattern_key}, DecisionConfidence(
        ...
        perf_with_isolation, perf_without_isolation = self.predict_performance_impact(features, True)
        ...
        reason = self._generate_decision_explanation(features, isolation_score, decision_threshold, confidence)
        ...
        decision_factors = {
            ...
            "dynamic_thresholds": {k: v.current_value for k, v in self.dynamic_thresholds.items()}
        }
        
        # Cache decision
        self.pattern_cache[pattern_key] = (enable_isolation, confidence.confidence_score, time.time())
    ...
    def should_enable_isolation_with_explanation(self, scenario: ScenarioContext) -> Tuple[bool, str, Dict[str, Any]]:
        """Wrapper method for backward compatibility with tests"""
        enable, reason, factors, confidence = self.should_enable_isolation_advanced(scenario)
    ...
    def _calculate_decision_confidence(self, features: AdvancedScenarioFeatures, isolation_score: float) -> DecisionConfidence:
        ...
        scenario = features.base_scenario
        if scenario.step_count >= 3 and scenario.context_size >= 5000 and scenario.estimated_tokens >= 10000:
            contributing_factors.append("consistent_complexity_indicators")
            base_confidence *= 1.1
        elif scenario.step_count <= 1 and scenario.context_size <= 1000 and scenario.estimated_tokens <= 3000:
    ...
    def _generate_decision_explanation(self, features: AdvancedScenarioFeatures, 
                                     isolation_score: float, threshold: float, 
                                     confidence: DecisionConfidence) -> str:
        """Generate human-readable explanation for the decision"""
        scenario = features.base_scenario
        
        if isolation_score >= threshold:
            explanation = f"启用隔离 (评分: {isolation_score:.1f} >= 阈值: {threshold:.1f})"
        else:
            explanation = f"不启用隔离 (评分: {isolation_score:.1f} < 阈值: {threshold:.1f})"
    ...
    def _get_feature_contributions(self, features: AdvancedScenarioFeatures) -> Dict[str, float]:
        ...
        contributions["complexity_score"] = features.query_complexity_score * self.feature_weights["complexity_score"]
        contributions["resource_pressure"] = features.resource_pressure * self.feature_weights["resource_pressure"]
        contributions["task_urgency"] = features.task_urgency_score * self.feature_weights["user_urgency"]
        
        return contributions
    
    def _record_decision(self, features: AdvancedScenarioFeatures, decision: bool, 
                        score: float, confidence: DecisionConfidence):
        ...
        decision_record = {
            ...
            "threshold_values": {k: v.current_value for k, v in self.dynamic_thresholds.items()}
        }
    ...
    def _auto_tune_parameters(self):
        ...
        logger.info(f"Auto-tuning completed. Updated {len(self.dynamic_thresholds)} thresholds")
    
    def record_scenario_outcome(self, scenario: ScenarioContext, isolation_enabled: bool, 
                              execution_time: float, token_savings: int = 0, 
        ...
        if hasattr(self, 'performance_predictor'):
            predicted_time, _ = self.performance_predictor.predict(features, isolation_enabled)
            self.performance_predictor.update_accuracy(execution_time, predicted_time)
        
        logger.debug(f"Recorded scenario outcome: isolation={isolation_enabled}, "
    ...
    def _adjust_feature_weights(self):
        ...
        for feature, importance in self.feature_importance.items():
            if feature in self.feature_weights:
                ...
                # Gradual adjustment
                self.feature_weights[feature] = current_weight + adjustment_rate * (target_weight - current_weight)
...
class PerformancePredictor:
    ...
    def predict(self, features: AdvancedScenarioFeatures, enable_isolation: bool) -> Tuple[float, float]:
        ...
        if enable_isolation:
            ...
            context_pollution_penalty = scenario.step_count * 0.1 + features.query_complexity_score * 0.2
    ...
    def update_accuracy(self, actual_performance: float, predicted_performance: float):
        ...
        error = abs(actual_performance - predicted_performance) / max(actual_performance, 0.1)
