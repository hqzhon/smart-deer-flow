...
class ResearcherProgressiveEnabler:
    ...
    def analyze_task_complexity(self, scenario: ScenarioContext) -> TaskComplexity:
        ...
        complex_keywords = [
            "analyze", "compare", "synthesize", "comprehensive", "detailed",
            "multiple sources", "cross-reference", "in-depth", "thorough"
        ]
    ...
    def should_enable_isolation(self, scenario: ScenarioContext) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if strategy == IsolationStrategy.DISABLED:
            ...
            return self._selective_enablement(scenario, complexity, config, decision_factors)
        
        elif strategy == IsolationStrategy.ADAPTIVE:
            return self._adaptive_enablement(scenario, complexity, config, metrics, decision_factors)
    ...
    def _selective_enablement(self, scenario: ScenarioContext, complexity: TaskComplexity, 
                            config: Optional[Any], decision_factors: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if scenario.step_count >= threshold:
            ...
            return True, f"Step count ({scenario.step_count}) exceeds threshold ({threshold})", decision_factors
        
        if scenario.context_size >= max_context:
            decision_factors["trigger"] = "context_size_threshold"
            return True, f"Context size ({scenario.context_size}) exceeds threshold ({max_context})", decision_factors
        ...
        if scenario.estimated_tokens >= 15000:
            ...
            return True, f"High token estimate ({scenario.estimated_tokens})", decision_factors
        
        decision_factors["trigger"] = "no_trigger_met"
        return False, "No selective enablement criteria met", decision_factors
    
    def _adaptive_enablement(self, scenario: ScenarioContext, complexity: TaskComplexity,
        ...
        if similar_scenarios:
            ...
            isolation_enabled_scenarios = [s for s in similar_scenarios if s[1]]  # enabled=True
            isolation_disabled_scenarios = [s for s in similar_scenarios if not s[1]]  # enabled=False
            
            if isolation_enabled_scenarios and isolation_disabled_scenarios:
                avg_perf_enabled = sum(s[2] for s in isolation_enabled_scenarios) / len(isolation_enabled_scenarios)
                avg_perf_disabled = sum(s[2] for s in isolation_disabled_scenarios) / len(isolation_disabled_scenarios)
                ...
                if avg_perf_enabled < avg_perf_disabled * (1 + self.performance_threshold):
                    decision_factors["trigger"] = "adaptive_performance_better"
                    return True, "Historical data shows isolation improves performance", decision_factors
        
        # Check current metrics
        health = metrics.get_isolation_health()
        
        if health["estimated_token_savings"] / max(health["total_isolation_sessions"], 1) >= self.token_savings_threshold:
            # Good token savings, enable for complex scenarios
            if complexity in [TaskComplexity.COMPLEX, TaskComplexity.VERY_COMPLEX]:
                decision_factors["trigger"] = "adaptive_good_savings"
                return True, "Good historical token savings for complex tasks", decision_factors
        ...
        if health["recent_success_rate"] >= 0.9:
            ...
            if scenario.step_count >= 2 or scenario.context_size >= 3000:
                ...
                return True, "High recent success rate allows aggressive enablement", decision_factors
        
        # Fall back to selective enablement
        return self._selective_enablement(scenario, complexity, config, decision_factors)
    
    def _find_similar_scenarios(self, scenario: ScenarioContext) -> List[Tuple[ScenarioContext, bool, float]]:
    ...
    def _calculate_similarity(self, scenario1: ScenarioContext, scenario2: ScenarioContext) -> float:
        ...
        step_similarity = max(0, 1 - step_diff / 5)  # Normalize by max expected difference
        similarity += step_similarity
        factors += 1
        
        # Context size similarity
        context_diff = abs(scenario1.context_size - scenario2.context_size)
        context_similarity = max(0, 1 - context_diff / 10000)  # Normalize by max expected difference
        ...
        token_similarity = max(0, 1 - token_diff / 20000)  # Normalize by max expected difference
    ...
    def record_scenario_outcome(self, scenario: ScenarioContext, isolation_enabled: bool, 
        ...
        if token_savings > 0:
            performance_score *= (1 - min(0.3, token_savings / 10000))  # Up to 30% improvement
        ...
        if not isolation_enabled:
            if self.performance_baseline is None:
                ...
                # Running average
                self.performance_baseline = (self.performance_baseline * 0.8 + performance_score * 0.2)
    ...
    def get_enablement_statistics(self) -> Dict[str, Any]:
        ...
        if enabled_scenarios:
            stats["avg_performance_enabled"] = sum(s[2] for s in enabled_scenarios) / len(enabled_scenarios)
        
        if disabled_scenarios:
            stats["avg_performance_disabled"] = sum(s[2] for s in disabled_scenarios) / len(disabled_scenarios)
        ...
        if self.user_feedback:
            stats["avg_user_satisfaction"] = sum(self.user_feedback.values()) / len(self.user_feedback)
