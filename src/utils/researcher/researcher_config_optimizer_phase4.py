...
class ConfigurationOptimizer:
    ...
    def _generate_intelligent_defaults(self) -> Dict[str, Any]:
        ...
        config = {
            "isolation": {
                ...
                "max_local_context_size": min(8000, int(memory_gb * 1000)),  # Scale with memory
        ...
        logger.info(f"Generated intelligent defaults for system: {cpu_count} CPUs, {memory_gb:.1f}GB RAM")
        return config
    ...
    def analyze_performance_data(self, metrics_data: Dict[str, Any]) -> List[ConfigRecommendation]:
        ...
        if avg_success_rate < 0.8:
            recommendations.append(ConfigRecommendation(
                ...
                current_value=self.current_config.get("performance", {}).get("success_rate_threshold", 0.8),
        ...
        overheads = [d["metrics"].get("performance_overhead_1h", 0.0) for d in recent_data]
        avg_overhead = statistics.mean(overheads)
        
        if avg_overhead > 0.3:
            current_sessions = self.current_config.get("isolation", {}).get("max_concurrent_sessions", 4)
        ...
        compression_ratios = [d["metrics"].get("avg_compression_ratio_1h", 1.0) for d in recent_data]
        avg_compression = statistics.mean(compression_ratios)
        
        if avg_compression > 0.8:  # Poor compression
            current_budget = self.current_config.get("isolation", {}).get("token_budget", 16000)
        ...
        utilizations = [d["metrics"].get("resource_utilization", 0.0) for d in recent_data]
        avg_utilization = statistics.mean(utilizations)
        
        if avg_utilization > 0.9:
            current_cleanup = self.current_config.get("performance", {}).get("memory_cleanup_interval", 300)
    ...
    def apply_recommendations(self, recommendations: List[ConfigRecommendation], 
        ...
        for rec in recommendations:
            if auto_apply or rec.confidence >= 0.8:
                ...
                logger.info(f"Applied config change: {rec.parameter.value} = {rec.recommended_value}")
    ...
    def apply_profile(self, profile_name: str) -> bool:
        ...
        logger.info(f"Applied profile '{profile_name}' with {len(applied_changes)} changes")
        return True
    ...
    def _filter_recommendations_by_level(self, recommendations: List[ConfigRecommendation]) -> List[ConfigRecommendation]:
        """Filter recommendations based on optimization level"""
        if self.optimization_level == ConfigOptimizationLevel.CONSERVATIVE:
            # Only high-confidence, low-impact changes
            return [r for r in recommendations if r.confidence >= 0.9 and r.impact_estimate == "low"]
    ...
    def get_configuration_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_profiles": {name: profile.to_dict() for name, profile in self.built_in_profiles.items()},
    ...
    def _get_system_recommendations(self) -> List[str]:
        ...
        if config_path.exists():
            ...
            if config_age_days > 30:
                recommendations.append(f"配置文件已 {config_age_days:.0f} 天未更新，建议检查是否需要优化")
    ...
    def enable_auto_tuning(self, optimization_level: ConfigOptimizationLevel = ConfigOptimizationLevel.BALANCED):
        ...
        self.current_config["optimization"]["optimization_level"] = optimization_level.value
    ...
    def export_config_backup(self, backup_file: Optional[str] = None) -> str:
        ...
        backup_file = backup_file or f"config_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
