...
class ExecutionContextManager:
    ...
    def _deduplicate_step_content(self, steps: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """去除步骤内容中的重复信息"""
        seen_content = set()
        deduplicated_steps = []
    ...
    def _format_context_info(self, steps: List[Dict[str, Any]], current_step: Dict[str, Any]) -> str:
        """格式化上下文信息"""
        if not steps:
            return "没有已完成的步骤。"
    ...
    def _simple_message_truncation(
        ...
        current_tokens = sum(count_tokens(msg.content).total_tokens for msg in system_messages)
        
        for msg in reversed(other_messages):
            msg_tokens = count_tokens(msg.content).total_tokens
            if current_tokens + msg_tokens <= token_limit:
                truncated_messages.insert(-len(system_messages) or len(truncated_messages), msg)
    ...
    def manage_observations_advanced(
        ...
        # 3. 选择最重要的观察结果
        selected = sorted(scored_observations, key=lambda x: x[1], reverse=True)[:max_observations]
        selected_observations = [obs for obs, score in selected]
        
        # 4. 内容压缩
        compressed = self._compress_observation_content(selected_observations, compression_ratio)
    ...
    def _extract_messages_from_args(self, args: tuple, kwargs: dict) -> List[BaseMessage]:
        """从函数参数中提取消息列表"""
        messages = []
        ...
        for key in ['messages', 'input', 'prompt']:
            if key in kwargs:
                ...
                if isinstance(value, list) and value and isinstance(value[0], BaseMessage):
                    ...
                    new_kwargs[key] = optimized_messages[0] if optimized_messages else value
                    return tuple(new_args), new_kwargs
