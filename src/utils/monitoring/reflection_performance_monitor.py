...
class ReflectionPerformanceMonitor:
    ...
    def _update_real_time_metrics(self, event: ReflectionEvent) -> None:
        ...
        try:
            ...
            if recent_events:
                ...
                self.current_metrics.reflection_accuracy = len(successful_events) / len(recent_events)
                
                gap_events = [e for e in recent_events if e.gaps_identified > 0]
                self.current_metrics.knowledge_gap_identification_rate = len(gap_events) / len(recent_events)
                ...
                if successful_events:
                    avg_confidence = sum(e.confidence for e in successful_events) / len(successful_events)
                    self.current_metrics.research_completeness_score = avg_confidence
                    
                    avg_sufficiency = sum(e.sufficiency_score for e in successful_events) / len(successful_events)
                    self.current_metrics.user_satisfaction_score = avg_sufficiency
                
                # Cache and efficiency metrics
                cache_events = [e for e in recent_events if e.metadata.get('cache_hit', False)]
                self.current_metrics.cache_hit_rate = len(cache_events) / len(recent_events) if recent_events else 0
    ...
    def _get_recent_events(self, hours: int = 24) -> List[ReflectionEvent]:
        ...
        return [event for event in self.events_history if event.start_time >= cutoff_time]
        ...
        with self.metrics_lock:
            ...
            dashboard = {
                ...
                'recent_activity': {
                    ...
                    'successful_events_24h': len([e for e in recent_events if e.success]),
                    'error_events_24h': len([e for e in recent_events if e.error_message]),
                    'avg_confidence_24h': self._calculate_average_confidence(recent_events),
                    'avg_gaps_identified_24h': self._calculate_average_gaps(recent_events)
                },
    ...
    def _assess_performance_status(self) -> str:
        ...
        if (metrics.reflection_accuracy >= thresholds['reflection_accuracy'] and
            metrics.knowledge_gap_identification_rate >= thresholds['knowledge_gap_identification_rate'] and
            metrics.error_rate <= thresholds['error_rate']):
            return 'excellent'
        elif (metrics.reflection_accuracy >= thresholds['reflection_accuracy'] * 0.9 and
              metrics.error_rate <= thresholds['error_rate'] * 1.5):
            return 'good'
    ...
    def _calculate_trends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Accuracy trend
            acc_1h = len([e for e in events_1h if e.success]) / len(events_1h) if events_1h else 0
            acc_24h = len([e for e in events_24h if e.success]) / len(events_24h) if events_24h else 0
            trends['accuracy_trend'] = self._determine_trend(acc_1h, acc_24h)
            
            # Error rate trend
            err_1h = len([e for e in events_1h if e.error_message]) / len(events_1h) if events_1h else 0
            err_24h = len([e for e in events_24h if e.error_message]) / len(events_24h) if events_24h else 0
            trends['error_rate_trend'] = self._determine_trend(err_24h, err_1h)  # Inverted for error rate
    ...
    def _check_performance_alerts(self) -> List[Dict[str, Any]]:
        ...
        if metrics.knowledge_gap_identification_rate < thresholds['knowledge_gap_identification_rate'] * 0.9:
            alerts.append({
                'level': 'warning',
        ...
        health_report = {
            ...
            'key_metrics': {
                'reflection_accuracy': {
                    ...
                    'status': 'pass' if dashboard['current_metrics']['reflection_accuracy'] >= self.target_thresholds['reflection_accuracy'] else 'fail'
                },
                'error_rate': {
                    'current': dashboard['current_metrics']['error_rate'],
                    'target': self.target_thresholds['error_rate'],
                    'status': 'pass' if dashboard['current_metrics']['error_rate'] <= self.target_thresholds['error_rate'] else 'fail'
                },
                'knowledge_gap_identification': {
                    'current': dashboard['current_metrics']['knowledge_gap_identification_rate'],
                    'target': self.target_thresholds['knowledge_gap_identification_rate'],
                    'status': 'pass' if dashboard['current_metrics']['knowledge_gap_identification_rate'] >= self.target_thresholds['knowledge_gap_identification_rate'] else 'fail'
                }
            },
    ...
    def _generate_recommendations(self, dashboard: Dict[str, Any]) -> List[str]:
        ...
        if metrics['reflection_accuracy'] < self.target_thresholds['reflection_accuracy']:
            recommendations.append("Consider tuning reflection model parameters or increasing training data quality")
        ...
        if metrics['cache_hit_rate'] < 0.5:
            recommendations.append("Optimize reflection caching strategy to improve performance")
        
        if dashboard['recent_activity']['total_events_24h'] < 10:
            recommendations.append("Low reflection activity detected - verify system integration")
...
def get_performance_monitor(config: Optional[Dict[str, Any]] = None) -> ReflectionPerformanceMonitor:
