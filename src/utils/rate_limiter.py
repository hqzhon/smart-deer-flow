...
class RateLimiter:
    ...
    def __init__(self, config: Optional[RateLimitConfig] = None):
        ...
        logger.info(f"Rate limiter initialized: {self.config.requests_per_minute}/min, {self.config.requests_per_second}/sec")
    ...
    async def acquire(self) -> float:
        ...
        if self.current_backoff > 0:
            backoff_remaining = self.current_backoff - (time.time() - self.last_failure_time)
            if backoff_remaining > 0:
                logger.info(f"Rate limiter in backoff, waiting {backoff_remaining:.2f}s")
        ...
        max_wait_time = max(wait_time_per_second, wait_time_per_minute, self.config.min_interval)
    ...
    def record_success(self):
        ...
        if self.failure_count > 0:
            logger.info(f"Request succeeded, resetting failure count from {self.failure_count}")
            self.failure_count = 0
            self.current_backoff = 0
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            average_delay = float(self.total_delay_time) / max(1, int(self.total_delays))
...
class AdaptiveRateLimiter(RateLimiter):
    ...
    def record_success(self):
        ...
        now = time.time()
        if (self.success_streak >= 10 and 
            now - self.last_adjustment > self.adjustment_interval and
            self.config.requests_per_second < self.max_requests_per_second):
            
            new_rate = min(self.max_requests_per_second, 
                          int(self.config.requests_per_second * 1.2))
    ...
    def record_failure(self, error_message: str = ""):
        ...
        if "rate limit" in error_message.lower() or "too many requests" in error_message.lower():
            new_rate = max(self.min_requests_per_second,
                          int(self.config.requests_per_second * 0.7))
            self.adjust_limits(requests_per_second=new_rate)
            self.last_adjustment = time.time()
            logger.info(f"Adaptive rate limiter decreased rate to {new_rate}/sec due to rate limit error")
