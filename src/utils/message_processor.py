...
def prepare_messages_with_context_evaluation(
    messages: list,
    ...
    try:
        ...
        if metrics.compression_needed or metrics.evaluation_result.value in [
            ...
            try:
                ...
                )  # Use configured limit instead of hardcoded

                if (
                    estimated_tokens > model_limit * 0.75
                ):  # 75% threshold for final check (reduced from 90%)
                    logger.warning(
                        f"Final token check: {estimated_tokens} tokens exceeds 75% of limit ({model_limit}) for model {model_name}"
                    )
                    # Emergency truncation with precise token counting
                    target_tokens = int(
                        model_limit * 0.7
                    )  # Target 70% of safe limit (more conservative)
                    ...
                    for msg in system_messages:
                        if hasattr(msg, "content"):
                            system_tokens += processor.estimate_tokens(
                                str(msg.content), model_name
                            )
                    ...
                    for msg in other_messages:
                        if hasattr(msg, "content"):
                            msg_tokens = processor.estimate_tokens(
                                str(msg.content), model_name
                            )
                            if current_tokens + msg_tokens <= remaining_tokens:
                                ...
                                if (
                                    ...
                                    if (
                                        best_content and len(best_content) > 50
                    ...
                    logger.info(
                        f"Emergency truncation applied: {len(messages)} -> {len(optimized_messages)} messages"
                    )
        ...
        logger.warning(
            f"Context evaluation failed: {e}, proceeding with original messages"
        )
        return messages
