...
class TokenManager:
    ...
    def validate_token_budget(
        ...
        if not content:
            return TokenValidationResult(
                is_valid=True, actual_tokens=0, max_tokens=max_tokens
            )
    ...
    def truncate_to_token_limit(
        ...
        if effective_limit < self.MIN_VIABLE_CONTENT_TOKENS:
            logger.warning(
                f"Token limit too small for meaningful content: {max_tokens}"
            )
        ...
        if preserve_start and preserve_end:
            return self._truncate_preserve_both_ends(
                content, effective_limit, model_name
            )
        elif preserve_start:
    ...
    def _truncate_preserve_both_ends(
        self, content: str, max_tokens: int, model_name: str
    ) -> str:
        ...
        return best_content or self._truncate_preserve_start(
            content, max_tokens, model_name
        )
    ...
    def _fallback_truncation(self, content: str, max_tokens: int) -> str:
        ...
        char_limit = int(
            max_tokens
            * self.CHAR_TO_TOKEN_RATIO_CONSERVATIVE
    ...
    def calculate_safe_token_limit(
        self, max_tokens: int, safety_margin: float = 0.1
    ) -> int:
    ...
    def split_content_by_tokens(
        self, content: str, chunk_size: int, model_name: str, overlap_tokens: int = 0
    ) -> list[str]:
        ...
        for sentence in sentences:
            ...
            sentence_tokens = self.count_tokens_precise(
                sentence_with_period, model_name
            )
                ...
                if current_chunk:
                    ...
                    if overlap_tokens > 0 and chunks:
                        overlap_content = self._extract_overlap(
                            current_chunk, overlap_tokens, model_name
                        )
                        current_chunk = overlap_content + sentence_with_period
                        current_tokens = self.count_tokens_precise(
                            current_chunk, model_name
                        )
                    else:
                        current_chunk = sentence_with_period
                        current_tokens = sentence_tokens
                    ...
                    truncated = self.truncate_to_token_limit(
                        sentence_with_period, chunk_size, model_name
                    )
    ...
    def _extract_overlap(
        self, content: str, overlap_tokens: int, model_name: str
    ) -> str:
