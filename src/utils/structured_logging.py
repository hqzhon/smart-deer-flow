...
class StructuredLogger:
    ...
    def _create_log_entry(self, level: LogLevel, message: str, event_type: EventType, 
                         data: Optional[Dict[str, Any]] = None, 
    ...
    def debug(self, message: str, event_type: EventType = EventType.SYSTEM, 
             data: Optional[Dict[str, Any]] = None, tags: Optional[List[str]] = None):
        """Debug log"""
        entry = self._create_log_entry(LogLevel.DEBUG, message, event_type, data=data, tags=tags)
        self._logger.debug(entry.to_json())
    
    def info(self, message: str, event_type: EventType = EventType.BUSINESS, 
            data: Optional[Dict[str, Any]] = None, tags: Optional[List[str]] = None):
        """Info log"""
        entry = self._create_log_entry(LogLevel.INFO, message, event_type, data=data, tags=tags)
        self._logger.info(entry.to_json())
    
    def warning(self, message: str, event_type: EventType = EventType.SYSTEM, 
               data: Optional[Dict[str, Any]] = None, tags: Optional[List[str]] = None):
        """Warning log"""
        entry = self._create_log_entry(LogLevel.WARNING, message, event_type, data=data, tags=tags)
    ...
    def error(self, message: str, error: Optional[Exception] = None, 
             ...
             data: Optional[Dict[str, Any]] = None, tags: Optional[List[str]] = None):
        """Error log"""
        entry = self._create_log_entry(LogLevel.ERROR, message, event_type, 
                                     data=data, error=error, tags=tags)
    ...
    def critical(self, message: str, error: Optional[Exception] = None, 
                ...
                data: Optional[Dict[str, Any]] = None, tags: Optional[List[str]] = None):
        """Critical error log"""
        entry = self._create_log_entry(LogLevel.CRITICAL, message, event_type, 
                                     data=data, error=error, tags=tags)
        self._logger.critical(entry.to_json())
    
    def performance(self, message: str, metrics: PerformanceMetrics, 
                   data: Optional[Dict[str, Any]] = None, tags: Optional[List[str]] = None):
        """Performance log"""
        entry = self._create_log_entry(LogLevel.INFO, message, EventType.PERFORMANCE, 
                                     data=data, performance=metrics, tags=tags)
        self._logger.info(entry.to_json())
    
    def request(self, message: str, data: Optional[Dict[str, Any]] = None, 
               tags: Optional[List[str]] = None):
        """Request log"""
        entry = self._create_log_entry(LogLevel.INFO, message, EventType.REQUEST, 
                                     data=data, tags=tags)
        self._logger.info(entry.to_json())
    
    def response(self, message: str, data: Optional[Dict[str, Any]] = None, 
                tags: Optional[List[str]] = None):
        """Response log"""
        entry = self._create_log_entry(LogLevel.INFO, message, EventType.RESPONSE, 
                                     data=data, tags=tags)
        self._logger.info(entry.to_json())
    
    def security(self, message: str, data: Optional[Dict[str, Any]] = None, 
                tags: Optional[List[str]] = None):
        """Security log"""
        entry = self._create_log_entry(LogLevel.WARNING, message, EventType.SECURITY, 
                                     data=data, tags=tags)
        self._logger.warning(entry.to_json())
...
def with_log_context(request_id: str = None, user_id: str = None, 
                    session_id: str = None, operation: str = None, 
                    component: str = None, trace_id: str = None):
...
def log_performance(operation: str = None, include_memory: bool = False, 
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.performance(
                    f"Operation failed: {operation or func.__name__}",
                    metrics,
                    data={'function': func.__name__, 'module': func.__module__, 'status': 'failed'}
                )
