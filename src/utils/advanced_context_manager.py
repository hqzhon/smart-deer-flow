...
class AdvancedContextManager:
    ...
    def create_context_segment(self, 
                             content: str,
                             segment_type: str,
                             priority: ContextPriority = ContextPriority.MEDIUM,
                             timestamp: Optional[float] = None) -> ContextSegment:
    ...
    def _calculate_importance_score(self, 
                                  content: str, 
                                  segment_type: str, 
                                  priority: ContextPriority) -> float:
        ...
        important_keywords = ['error', 'warning', 'critical', 'important', 'required', 'must']
    ...
    def optimize_context_for_parallel(self, 
                                    completed_steps: List[Any],
                                    current_task: str,
                                    model_name: str,
                                    strategy: CompressionStrategy = CompressionStrategy.ADAPTIVE) -> ContextWindow:
        """Optimize context specifically for parallel execution"""
        logger.info(f"Optimizing context for parallel execution with strategy: {strategy.value}")
        ...
        compression_ratio = 1.0 - (total_tokens / sum(seg.token_count for seg in segments))
        ...
        logger.info(f"Context optimization complete: {total_tokens}/{max_context_tokens} tokens, "
    ...
    def _apply_compression_strategy(self, 
                                  segments: List[ContextSegment],
                                  max_tokens: int,
                                  strategy: CompressionStrategy,
                                  model_name: str) -> List[ContextSegment]:
        ...
        if strategy == CompressionStrategy.ADAPTIVE:
            strategy = self._select_adaptive_strategy(segments, max_tokens, current_tokens)
        ...
        if strategy == CompressionStrategy.TRUNCATE:
            ...
            return self._apply_sliding_window(segments, max_tokens)
        elif strategy == CompressionStrategy.HIERARCHICAL:
            return self._apply_hierarchical_selection(segments, max_tokens)
        elif strategy == CompressionStrategy.SUMMARIZE:
            return self._apply_summarization(segments, max_tokens, model_name)
        else:
    ...
    def _select_adaptive_strategy(self, 
                                segments: List[ContextSegment],
                                max_tokens: int,
                                current_tokens: int) -> CompressionStrategy:
    ...
    def _apply_truncation(self, 
                         segments: List[ContextSegment],
                         max_tokens: int) -> List[ContextSegment]:
        ...
        for segment in sorted_segments:
            if current_tokens + segment.token_count <= max_tokens:
                ...
                if remaining_tokens > 100:  # Minimum viable segment size
                    truncated_content = self._truncate_content(segment.content, remaining_tokens)
                    truncated_segment = ContextSegment(
                        ...
                        importance_score=segment.importance_score * 0.8  # Reduce score for truncated
                    )
    ...
    def _apply_sliding_window(self, 
        ...
        critical_segments = [seg for seg in segments if seg.priority == ContextPriority.CRITICAL]
        other_segments = [seg for seg in segments if seg.priority != ContextPriority.CRITICAL]
    ...
    def _apply_hierarchical_selection(self, 
                                    segments: List[ContextSegment],
                                    max_tokens: int) -> List[ContextSegment]:
        ...
        for priority in [ContextPriority.CRITICAL, ContextPriority.HIGH, 
            ...
            group_segments = sorted(priority_groups[priority], 
                ...
                if used_tokens + segment.token_count <= max_tokens:
                    ...
                    if segment.can_compress and priority != ContextPriority.CRITICAL:
                        ...
                        if remaining_tokens > 50:
                            compressed_content = self._compress_segment_content(segment.content, remaining_tokens)
...
Summary (keep under {remaining_tokens // 4} words):"""
            ...
            try:
                ...
                summarized_content = self._compress_segment_content(combined_content, remaining_tokens)
    ...
    def get_context(self, completed_steps: List[Any], current_task: str, model_name: str = "deepseek-chat", max_tokens: Optional[int] = None) -> str:
        ...
        try:
            ...
            if max_tokens is not None:
                ...
                context_max_tokens = int(limits.safe_input_limit * self.max_context_ratio)
    ...
    def get_optimization_stats(self, context_window: ContextWindow) -> Dict[str, Any]:
        ...
        return {
            ...
            'segments_by_priority': {
                priority.name: len([seg for seg in context_window.segments if seg.priority == priority])
                for priority in ContextPriority
            },
            'segments_by_type': {
                seg_type: len([seg for seg in context_window.segments if seg.segment_type == seg_type])
