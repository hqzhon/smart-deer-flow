...
class TokenCounter:
    ...
    def get_encoding_for_model(self, model_name: str) -> str:
        ...
        for known_model, encoding in self.MODEL_ENCODINGS.items():
            if known_model.lower() in model_lower or model_lower in known_model.lower():
                logger.debug(f"Using encoding {encoding} for model {model_name} (matched with {known_model})")
                return encoding
        
        # Default encoding for unknown models
        logger.warning(f"Unknown model {model_name}, using default encoding cl100k_base")
        return "cl100k_base"
    ...
    def count_message_tokens(self, messages: List[BaseMessage], model_name: str = "gpt-4") -> TokenCountResult:
    ...
    def check_token_limit(self, 
                         ...
                         model_name: str, 
                         max_tokens: int, 
                         safety_margin: float = 0.9) -> tuple[bool, TokenCountResult]:
    ...
    def estimate_output_tokens(self, input_text: str, model_name: str, compression_ratio: float = 0.3) -> int:
...
def count_message_tokens(messages: List[BaseMessage], model_name: str = "gpt-4") -> TokenCountResult:
    """Count tokens in messages"""
    return get_token_counter().count_message_tokens(messages, model_name)


def check_token_limit(text: str, model_name: str, max_tokens: int, safety_margin: float = 0.9) -> tuple[bool, TokenCountResult]:
    """Check if text exceeds token limit"""
    return get_token_counter().check_token_limit(text, model_name, max_tokens, safety_margin)