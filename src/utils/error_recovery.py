...
class ErrorRecoveryManager:
    ...
    async def execute_with_recovery(
        self, func: Callable, service_name: str, *args, **kwargs
    ) -> Any:
        """Execute with recovery mechanism"""
        strategy = self.recovery_strategies.get(
            service_name, RecoveryStrategy.EXPONENTIAL_BACKOFF
        )
        ...
        if strategy == RecoveryStrategy.CIRCUIT_BREAKER and circuit_breaker:
            if not circuit_breaker.can_execute():
                logger.warning(
                    f"Circuit breaker is OPEN for {service_name}, trying degraded service"
                )
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                logger.debug(
                    f"Service {service_name} executed successfully on attempt {attempt + 1}"
                )
                return result
                ...
                logger.warning(
                    f"Service {service_name} failed on attempt {attempt + 1}: {e}"
                )
        ...
        if self.config.enable_degraded_service:
            logger.info(
                f"All retries failed for {service_name}, trying degraded service"
            )
            try:
                return await self._try_degraded_service(service_name, *args, **kwargs)
            except Exception as degraded_error:
                logger.error(
                    f"Degraded service also failed for {service_name}: {degraded_error}"
                )
