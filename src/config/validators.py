...
def validate_llm_config(config: Dict[str, Any]) -> bool:
    ...
    # Check required fields based on LLMSettings model
    required_fields = ['basic_model']
    for field in required_fields:
        if field not in llm_config:
            logger.error(f"Missing required LLM field: {field}")
            return False
        # Check that the field is not empty
        if not llm_config[field]:
            logger.error(f"LLM field cannot be empty: {field}")
            return False
...
def validate_agent_config(config: Dict[str, Any]) -> bool:
    ...
    if not isinstance(max_iterations, int) or max_iterations <= 0:
        logger.error(f"Invalid max_iterations: {max_iterations}. Must be positive integer")
        return False
...
def validate_file_paths(config: Dict[str, Any]) -> bool:
    ...
    if database_config.get('type') == 'sqlite':
        ...
        if db_path:
            paths_to_check.append(('database.path', db_path, True))  # True means create if not exists
