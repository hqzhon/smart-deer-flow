...
class WebSearchTool(BaseTool):
        ...
        try:
            ...
            if search_engine == "tavily":
                return self._search_tavily(
                    query, max_results, include_raw_content, include_images
                )
            ...
            return ToolOutput(
                success=False, message=f"Search failed: {str(e)}", data=[]
            )
    ...
    def _search_tavily(
        ...
        try:
            ...
            return ToolOutput(
                success=True, message=f"Found {len(results)} results", data=results
            )

        except Exception as e:
            logger.error(f"Tavily search failed: {e}")
            return ToolOutput(
                success=False, message=f"Tavily search failed: {str(e)}", data=[]
            )
    ...
    def _search_duckduckgo(self, query: str, max_results: int) -> ToolOutput:
        ...
        try:
            ...
            return ToolOutput(
                success=False, message=f"DuckDuckGo search failed: {str(e)}", data=[]
            )
    ...
    def _search_brave(self, query: str, max_results: int) -> ToolOutput:
        ...
        try:
            ...
            if not api_key:
                return ToolOutput(
                    success=False, message="Brave Search API key not found", data=[]
                )

            search_tool = BraveSearch(
                search_wrapper=BraveSearchWrapper(
                    api_key=api_key, search_kwargs={"count": max_results}
                )
            )
            ...
            return ToolOutput(
                success=False, message=f"Brave search failed: {str(e)}", data=[]
            )
...
class ArxivSearchTool(BaseTool):
    ...
    def execute(self, query: str, max_results: int = 5) -> ToolOutput:
        ...
        try:
            ...
            return ToolOutput(
                success=False, message=f"ArXiv search failed: {str(e)}", data=[]
            )
