...
class ParallelOptimizationBenchmark:
    ...
    async def run_research_benchmark(self, 
                                    query: str, 
                                    optimization_levels: List[WorkflowOptimizationLevel] = None) -> List[BenchmarkResult]:
        """运行研究工作流基准测试"""
        ...
        for level in optimization_levels:
            ...
            result = await run_optimized_research_workflow(
                ...
                enable_intelligent_task_decomposition=(level.value in ['advanced', 'maximum']),
            ...
            print(f"CPU使用率: {benchmark_result.cpu_usage:.1f}%, 内存使用: {benchmark_result.memory_usage:.1f}MB")
            print(f"任务总数: {benchmark_result.task_count}, 并行任务: {benchmark_result.parallel_tasks}")
    ...
    async def run_report_generation_benchmark(self, 
        ...
        for level in optimization_levels:
            ...
            print(f"完成报告生成 {level.value} 级别测试，执行时间: {execution_time:.2f}秒")
            print(f"CPU使用率: {benchmark_result.cpu_usage:.1f}%, 内存使用: {benchmark_result.memory_usage:.1f}MB")
    ...
    def generate_report(self) -> str:
        ...
        report.append("| 优化级别 | 平均执行时间(秒) | CPU使用率(%) | 内存使用(MB) | 并行任务数 |")
        report.append("| --- | --- | --- | --- | --- |")
        ...
        for level in ['basic', 'standard', 'advanced', 'maximum']:
            if level in results_by_level:
                ...
                avg_parallel = sum(r.parallel_tasks for r in level_results) / len(level_results)
                
                report.append(f"| {level.capitalize()} | {avg_time:.2f} | {avg_cpu:.1f} | {avg_memory:.1f} | {avg_parallel:.1f} |")
        
        # 性能提升分析
        if 'basic' in results_by_level and 'maximum' in results_by_level:
            basic_avg_time = sum(r.execution_time for r in results_by_level['basic']) / len(results_by_level['basic'])
            max_avg_time = sum(r.execution_time for r in results_by_level['maximum']) / len(results_by_level['maximum'])
            time_improvement = ((basic_avg_time - max_avg_time) / basic_avg_time) * 100
            
            report.append(f"\n## 性能提升分析\n")
            report.append(f"* 基础优化到最大优化的执行时间减少: **{time_improvement:.1f}%**")
        ...
        if recent_results:
            ...
            for i, result in enumerate(recent_results):
                report.append(f"### 测试 {i+1} ({result.optimization_level.capitalize()})\n")
...
async def main():
    ...
    if choice == "1":
        query = input("请输入研究查询: ") or "分析人工智能在医疗领域的应用和未来发展趋势"
